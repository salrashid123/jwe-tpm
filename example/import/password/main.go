...
var (
	...
	password         = flag.String("password", "foooooo", "Password for the created key")
...
func run() int {
	...
	tc, err := jwetpm.NewPolicyAuthValueAndDuplicateSelectSession(rwr, []byte(*password), primaryKey.Name, primaryKey.ObjectHandle)
	if err != nil {
		log.Fatalf("error creating NewPolicyAuthValueAndDuplicateSelectSession %v", err)
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "  error decrypting: %v", err)
		return 1
	}
